        ...
        <div class="container mx-auto bg-gray-200 flex font-sans">
            <div class="w-1/5 bg-gray-300 text-center p-6">
                <button
                    ...
                    @click="ShowStoreDiscussionModel">
                    ask a question
                </button>
                <modal name="storeDiscussionModel" height="630">
                    ...
                    <form @submit.prevent="storeDiscussion">
                        ...
                        v-model="dataForm.channel_id">
                            ...
                            <option v-for="channel in channels.data" :key="channel.id" :value="channel.id">
                                {{channel.title}}
                            </option>
                        </select>
                        <vue-editor v-model="dataForm.contentt"></vue-editor>
                        <input type="hidden" :value="dataForm.contentt">
                        <button
                            class="bg-gray-700 text-white py-2 px-4 rounded-full uppercase  text-sm"
                            @click="HideStoreDiscussionModel">Canncel</button>
                        <button
                            class="bg-btnBlueColor text-white py-2 px-4 rounded-full uppercase  text-sm" type="submit">
                            New Discussion
                        </button>
              ...
              <label href="" class="my-4 block font-black capitalize">all channels</label>
                <ul>
...
<script>
    ...
    // mapGetters this function goes to store and search for the Appropriate getters
    export default {
        data() {
            return {
                dataForm: {
        ...
        methods: {
            ShowStoreDiscussionModel() {
                this.$modal.show('storeDiscussionModel');

            },
            HideStoreDiscussionModel(){
                this.$modal.hide('storeDiscussionModel');

            },
            storeDiscussion(){
                let data={
                    title:this.dataForm.title,
                    contentt:this.dataForm.contentt,
